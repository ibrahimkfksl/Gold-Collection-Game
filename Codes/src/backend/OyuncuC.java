...
public class OyuncuC {
	...
	public OyuncuC(int toplam_altin, OyunMasasi oyun_masasi, int adim_maliyeti, int hedef_belirleme_maliyeti,
			int adim_sayisi) {
		...
		OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.BLUE);

		elendi_mi = false;

		cikti = new File("Oyuncu_C.txt");
		BufferedWriter br2;
		try {
			...
			System.out.println("C Oyuncusunda Dosyaya Yazma hatasi");

		}

	}

	private void gizliAcigaCikar() {
		...
		for (int k = 0; k < 2; k++) {
			...
			int kazanc = (mesafe * -1) * adim_maliyeti;// buna farkli bir deger de atanabilir sayi cok kucuk oldugu
														// surece
														// sikint yok
			...
			for (int i = 0; i < masa_y; i++) {
				for (int j = 0; j < masa_x; j++) {
					...
					if (oyun_masasi.getMasa()[i][j] < 0) {
						...
						int yeni_kazanc = (oyun_masasi.getMasa()[i][j] * -1) - (yeni_mesafe * adim_maliyeti);
			...
			if (hedef[0] != -1) {
				if (k == 0) {
					try {
						br.write("Aciga Cikarilan Gizli Altinlar-----> ");
					} catch (IOException e) {
						System.out.println("C Oyuncusunda Dosyaya Yazma Hatasi");
					}
				}

				cikanGizliAltinlariYaz(hedef[0], hedef[1]);
				oyun_masasi.gizliAltinOrtayaCikar(hedef[0], hedef[1]);
			}
	...
	public void hedefBelirle() {
		...
		yakin_hedef[0] = gecici_hedef[0];
		yakin_hedef[1] = gecici_hedef[1];
	...
	public int hareketEt() {
		...
		if ((yakin_hedef[0] == -1) || !hedefKontrolEt(yakin_hedef[0], yakin_hedef[1])) {
			...
			if ((yakin_hedef[0] == -1) && (yakin_hedef[1] == -1)) {
				return 2;
			} else {
		...
		for (int i = 0; i < adim_sayisi; i++) {
			...
			adim_eksilt = false;
			if (yon == 1) {
				if (yakin_hedef[0] != bulunan_nokta[0]) {
					if (yakin_hedef[0] < bulunan_nokta[0]) {
					...
					} else if (yakin_hedef[0] > bulunan_nokta[0]) {
						if (!oyuncuOnumdeMi(bulunan_nokta[0] + 1, bulunan_nokta[1])) {
							OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.WHITE);
							bulunan_nokta[0] += 1;
							OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.BLUE);
				...
				if (yakin_hedef[1] != bulunan_nokta[1]) {
					if (yakin_hedef[1] < bulunan_nokta[1]) {
					...
					} else if (yakin_hedef[1] > bulunan_nokta[1]) {
						if (!oyuncuOnumdeMi(bulunan_nokta[0], bulunan_nokta[1] + 1)) {
							OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.WHITE);
							bulunan_nokta[1] += 1;
							OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.BLUE);
		...
		if ((yakin_hedef[0] == bulunan_nokta[0]) && (yakin_hedef[1] == bulunan_nokta[1])) {
			toplam_altin += oyun_masasi.getMasa()[bulunan_nokta[1]][bulunan_nokta[0]];
		...
		try {
			br.write("C Turunu Tamamladi. Sira Diger Oyuncuda");
			br.newLine();
			br.newLine();
		} catch (IOException e) {
			System.out.println("C Oyuncusunda Dosyaya Yazma Hatasi");
	...
	private void eleniceDosyayaYaz() {
		...
		if (!elendi_mi) {
			try {
				br.newLine();
				br.write("C Oyuncusu Altini Yeterli Olmadigi Icin Elenmistir");
				br.newLine();
	...
	private void yeniHedefiDosyayaYaz() {
		try {
			if ((yakin_hedef[0] == -1) && (yakin_hedef[1] == -1)) {
				br.write("C Yeni Hedef Bulamadi. Turun Bitmesini Bekliyor...");
				br.newLine();
				br.newLine();
			} else {

				br.write("Yeni Hedef Bulundu  X:" + yakin_hedef[0] + " Y:" + yakin_hedef[1]);
				br.newLine();
	...
	private void hareketOncesiKonumuYazdir() {
		try {
			br.write("Son Konum: (" + bulunan_nokta[0] + "," + bulunan_nokta[1] + ")------->Hedef: (" + yakin_hedef[0]
					+ "," + yakin_hedef[1] + ")");
			br.newLine();
		} catch (IOException e) {
			System.out.println("C Oyuncusunda Dosyaya Yazma Hatasi");
	...
	/*
	 * C nin gucu olan gizli altinlari aciga cikarirken dosyayada yaziyor
	 */
	private void cikanGizliAltinlariYaz(int yeni_x, int yeni_y) {
		try {
			br.write("(" + yeni_x + "," + yeni_y + ")-----");
		} catch (IOException e) {
			System.out.println("C Oyuncusunda Dosyaya Yazma Hatasi");
	...
	public void oyunculariTanit(OyuncuA oyuncu_a, OyuncuB oyuncu_b, OyuncuD oyuncu_d) {
	...
	public int yonDegistir(int yon) {
		if ((yon == 1) && (bulunan_nokta[1] != yakin_hedef[1])) {
			return 2;
		} else if ((yon == 2) && (bulunan_nokta[0] != yakin_hedef[0])) {
			return 1;
		} else if (((yon == 1) && (bulunan_nokta[1] == yakin_hedef[1]))) {
