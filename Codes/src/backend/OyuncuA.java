...
public class OyuncuA {
	...
	public OyuncuA(int toplam_altin, OyunMasasi oyun_masasi, int adim_maliyeti, int hedef_belirleme_maliyeti,
			int adim_sayisi) {
		...
		OyunEkrani.board[bulunan_nokta[1]][bulunan_nokta[0]].setFill(Color.RED);
	...
	/*
	 * Hedefi belirlerken en yakin mesafeyi hesaplamak icin indislerle islem
	 * yap�ld� Ilk olarak hipotenus uzunlugunu ele alarak yaptik dogru sonuc
	 * vermedi Bu yuzdende sadece x ve y dogrultularinda gidebilecegi icin bu
	 * dogrultularda adim sayilari ele alinarak mesafe hesaplandi
	 */
	...
	public int hareketEt() {
		...
		try {
			br.write("A Turunu Tamamladi. Sira Diger Oyuncuda");
			br.newLine();
			br.newLine();
		} catch (IOException e) {
			System.out.println("A Oyuncusunda Dosyaya Yazma Hatasi");
	...
	private void eleniceDosyayaYaz() {
		...
		if (!elendi_mi) {
			try {
				...
				br.write("A Oyuncusu Altini Yeterli Olmadigi Icin Elenmistir");
				br.newLine();
	...
	private void yeniHedefiDosyayaYaz() {
		try {
			if ((hedef[0] == -1) && (hedef[1] == -1)) {
				br.write("A Yeni Hedef Bulamadi. Turun Bitmesini Bekliyor...");
				br.newLine();
				br.newLine();
			} else {
	...
	public void oyunculariTanit(OyuncuB oyuncu_b, OyuncuC oyuncu_c, OyuncuD oyuncu_d) {
